/* ============================================================
   ...
   ------------------------------------------------------------
   DATA LAYER ახლა backend API-ს იძახებს (/api/reports), არა
   localStorage-ს — ანუ ერთის შეტყობინება ყველას ერთად ჩაუსმენს.
   API-ც ის "DATA LAYER" კონცეფციით აგებულია, რაც თავიდან იყო:
   loadReports/getReport/setReport ფუნქციების სიგნატურა იგივეა,
   მხოლოდ შიგნით ახლა fetch()-ია localStorage-ის ნაცვლად.

   STOPS მასივი იტვირთება stops.js-დან (routes_stops.shp +
   minibus_stops.shp -ის გაერთიანებული, WGS84-ში გადაყვანილი
   მონაცემები). ყოველი ჩანაწერია:
   { id, name, lat, lng, types: ["bus","minibus"], routes: [...] }
   ============================================================ */

const STOPS_BY_ID = {};
STOPS.forEach((s) => (STOPS_BY_ID[s.id] = s));

/* ---------- DATA LAYER (ახლა: backend API) ----------
   reportsCache - ბოლო ცნობილი მონაცემები სერვერიდან, რომელსაც
   მარკერების render-ი სინქრონულად კითხულობს, რომ UI არ "აციმციმდეს"
   ყოველი fetch-ის მოლოდინში. */
const API_BASE = "/api";
let reportsCache = {};
...
async function setReport(stopId, status) {
...
/* 2700+ გაჩერებაა — ცალ-ცალკე მარკერების ნაცვლად, ვაჯგუფებთ
   კლასტერებად, რომ რუკა სწრაფად იხსნებოდეს და დიდი მასშტაბიდან
   გადატვირთული არ იყოს. მონიშნულ (წითელი/მწვანე) გაჩერებებს
   კლასტერშიც გამოვყოფთ ფერით. */
const clusterGroup = L.markerClusterGroup({
  ...
  iconCreateFunction: (cluster) => {
    ...
    const cls = hasInspector ? "clusterIcon clusterIcon--alert" : "clusterIcon";
    return L.divIcon({
...
/* ---------- პერიოდული სინქრონიზაცია სერვერთან ----------
   ...
   ყოველ 15 წამში ერთხელ poll-ის წყალობით. შუაღამის გასუფთავებას
   სერვერი თავად ამუშავებს (report_date ფილტრით) — client-ი
   უბრალოდ კითხავს განახლებულ შედეგს. */
